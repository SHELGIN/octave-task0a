...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  eq1 = x1_dot;
  eq2 = x2_dot;
  J = jacobian([eq1, eq2], [x1, x2]);
    for i = 1:size(solutions, 1)
        eqbm = solutions(i, :);
        J_substituted = subs(J, [x1, x2], eqbm);
        jacobian_matrices{i} = J_substituted;
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    flag = 1;
    eig_values = eig(matrix);
    eigen_values{k} = double(eig_values);
    for i = 1:length(eig_values)
      if real(eig_values(i)) >= 0
        flag = 0; 
        break;
      end
    end
